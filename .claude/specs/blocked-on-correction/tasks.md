...
- [ ] T1: clear the false block — files: .claude/specs/cpd-path-proven/status.yml — AC: AC1, AC2, AC3
...
- [ ] T2: BLOCKED requires probe evidence — files: plugins/quorum-orchestrator/skills/quorum-status/SKILL.md, scripts/e2e.py — AC: AC4, AC7
...
- [ ] T3: the rule reaches every precondition evaluator — files: plugins/quorum-orchestrator/governance/rules/GLOBAL.md, scripts/e2e.py — AC: AC5, AC7
...
- [ ] T4: sweep existing records — files: scripts/check.py — AC: AC6
      A check that finds any status.yml claiming a precondition without probe
      evidence. Reports; never corrects.

## Phase 4 — the gate

- [ ] T5: run both suites — files: (gate; no source changes) — AC: AC7
