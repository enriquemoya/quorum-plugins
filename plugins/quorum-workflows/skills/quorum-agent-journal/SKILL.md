...
Vault root: `$QUORUM_VAULT` (default `~/quorum-vault`). Scaffolder: `<obsidian-skill-dir>\scripts\scaffold_vault.ps1` (idempotent — "never clobbers"). **Resolve `<obsidian-skill-dir>` at runtime — never type the literal token:** Glob for `**/quorum-obsidian-vault/scripts/scaffold_vault.ps1` and use that absolute path (fallback: `~/.claude/plugins/cache/quorum-plugins/quorum-workflows/1.0.0/skills/quorum-obsidian-vault/scripts/scaffold_vault.ps1`). If neither resolves, STOP and report — do not skip the scaffold silently (the story note + AC matrix would never be created).
...
3. Report what was promoted and to where. Edits land in your local checkout of this marketplace, under `plugins/`. **After editing, sync the plugin cache or the change never loads** — copy the edited plugin into `~/.claude/plugins/cache/quorum-plugins/<plugin>/<version>/`, then reload Claude Code for the new text to take effect. An unsynced live edit is a silent no-op.
