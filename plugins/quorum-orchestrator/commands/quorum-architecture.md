---
argument-hint: "<slug>"
description: Stage 3 of the governed pipeline. Produces architecture.md — components touched, data and contract deltas, boundaries, failure modes and rollback. Invoked by /quorum-orchestrate; reads the profile for every stack fact.
---

# /quorum-architecture

Produces `architecture.md`. Runs on DRAFTING_SPEC.

## Process

1. Delegate to the **quorum-architecture** agent.
...
4. It writes `architecture.md` and transitions to `DRAFTING_ARCH` through `quorum-status`.
