...
description: Generate a structured investigation prompt for a Jira ticket. Fetches the ticket, analyzes requirements, recommends memory bank patterns, and produces a stack-agnostic implementation/test-authoring guide.
---

# Prompt Generator Command

Generate a structured investigation prompt from a Jira ticket. Content is shaped by the consumer repo's `.claude/profile.yml`, so any project (frontend, backend, QA-automation, etc.) can reuse this command — the prompt only references roles that the consumer has actually configured.
...
1. Fetches ticket from Jira (`getJiraIssue`)
...
5. Saves to `.claude/prompts/{TICKET-KEY}-{DATE}.md` (transient working copy — gitignored; in the orchestrator pipeline the prompt is posted to the Jira ticket as a comment)

## Output Sections

1. Ticket Overview (routes, roles, complexity, stack context resolved from profile)
2. Problem Statement (AC from Jira + Visual Spec subsection if image analysis ran)
